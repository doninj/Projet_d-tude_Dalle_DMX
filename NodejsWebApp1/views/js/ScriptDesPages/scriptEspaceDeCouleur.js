...
window.HTMLCIEXyChart = class HTMLCIEXyChart extends HTMLElement {
    ...
    constructor() {
        ...
        this.addEventListener('mousedown', function (e) {
            if (e.button !== 0)
                return;
            this.mousemoveWhileMouseDown(e);
            this.addEventListener('mousemove', this.mousemoveWhileMouseDown);
        });
        this.addEventListener('mouseup', function (e) {
            if (e.button !== 0)
                return;
            this.removeEventListener('mousemove', this.mousemoveWhileMouseDown);
            this.mousemoveWhileMouseDown(e);
        });
        this.addEventListener('mousemove', function (e) {
            this.canvas.style.cursor = this.context.isPointInPath(this.λPath, e.offsetX, e.offsetY) ? 'crosshair' : 'auto';
        });
        this.ready = true;
        this.buildStyle();
        this.redraw();
    ...
    //dessine le quadrillage et les axes
    drawReference() {
        ...
        measure = this.context.measureText(str);
        width = Math.abs(measure.actualBoundingBoxLeft - measure.actualBoundingBoxRight);
        this.context.fillText('x', (this.width + this.axesFreeSpace * this.width / this.divisionNumber) / 2 - width / 2, this.height - 5);
        str = 'y';
        measure = this.context.measureText(str);
        height = Math.abs(measure.actualBoundingBoxAscent - measure.actualBoundingBoxDescent);
        this.context.fillText('y', 5, (this.height - this.axesFreeSpace * this.height / this.divisionNumber) / 2 + height / 2);
        this.context.stroke();
    }
    ...
    fillInputs(changeSource) {
        ...
        this.colorDiv.style.height = "150px";
        this.colorDiv.style.width = "150px";
    }
    ...
    // Events:
    mousemoveWhileMouseDown(e) {
        if (this.context.isPointInPath(this.λPath, e.offsetX, e.offsetY)) {
            const x = this.XTox(e.offsetX);
            const y = this.YToy(e.offsetY);
            this.pointer = x + ':' + y;
            this.inputs.xy.x.safeValue = x;
            this.inputs.xy.y.safeValue = y;
            this.fillInputs('pointer');
        }
    }
...
var commandControlerDalle = {
    command: "C_CommanderEspaceCouleur",
    universe: 1,
    numeroCaisson: 1,
    dataEspaceCouleurCIE: [],
...
let tabCurseurBleu = ["id_nm_490", "id_nm_390", "id_nm_403", "id_nm_410", "id_nm_440"];
let tabCurseurRouge = ["id_nm_590", "id_nm_605", "id_nm_655", "id_nm_620", "id_nm_630"];
let tabCurseurVert = ["id_nm_502", "id_nm_512", "id_nm_520", "id_nm_530"];
...
//Appel une fonction dans un autre fichier (PageCouleur.ejs) pour récupérer l'id du curseur rouge qui est utilisé 
//et ajuste les curseau pour les mettre a 0
//Paramètre d'entrée  : Aucune
//Valeur retournée : Aucune
function ajustCursorRouge() {

    setTimeout(() => {
        rouge = vm.getIdCurseurRouge();
        for (let i = 0; i < tabCurseurRouge.length; i++) {

            commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(tabCurseurRouge[i])] = 0;
            console.log("ok")
        }
        commandControlerDalle.dataEspaceCouleurCIE[76] = 0
        commandControlerDalle.dataEspaceCouleurCIE[78] = 0 //pas d'erreur
        commandControlerDalle.rouge = rouge;
        console.log("rouge " + vm.getIdCurseurRouge());
...
//Appel une fonction dans un autre fichier (PageCouleur.ejs) pour récupérer l'id du curseur vert qui est utilisé
//et ajuste les curseau pour les mettre a 0
//Paramètre d'entrée  : Aucune
//Valeur retournée : Aucune
function ajustCursorVert() {

    setTimeout(() => {
        vert = vm.getIdCurseurVert();
        for (let i = 0; i < tabCurseurVert.length; i++) {
            commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(tabCurseurVert[i])] = 0;

        }
        console.log("vert " + vm.getIdCurseurVert());
        commandControlerDalle.vert = vert;
    }, 50)
...
//Paramètre d'entrée  : Aucune
//Valeur retournée : Aucune
function ajustCursorBleu() {

    setTimeout(() => {
        bleu = vm.getIdCurseurBleu();
        for (let i = 0; i < tabCurseurBleu.length; i++) {

            commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(tabCurseurBleu[i])] = 0;
        }
        console.log("blue " + vm.getIdCurseurBleu());
        commandControlerDalle.bleu = bleu;
    }, 50)
...
//valeur retournée : Aucune 
function commanderColorR() {

    for (let i = 0; i < tabCurseurRouge.length; i++) {

        commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(tabCurseurRouge[i])] = 0;
        console.log("ok")
        if (tabCurseurRouge[i] === rouge) {
            commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(rouge)] = parseInt(document.getElementById(rouge).value);
            console.log("Rouge  ::" + parseInt(document.getElementById(rouge).value));
    ...
    try {
        commandControlerDalle.dataEspaceCouleurCIE[76] = parseInt(document.getElementById("id_nm_605").value);
        commandControlerDalle.dataEspaceCouleurCIE[78] = parseInt(document.getElementById("id_nm_605").value); //pas d'erreur
    }
    catch {
...
//Paramètre d'entrée  : Aucun
//valeur retournée : Aucune 
function commanderColorV() {
    for (let i = 0; i < tabCurseurVert.length; i++) {

        commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(tabCurseurVert[i])] = 0;
        if (tabCurseurVert[i] === vert) {
            commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(vert)] = parseInt(document.getElementById(vert).value);
            console.log("Vert  ::" + parseInt(document.getElementById(vert).value));
        }
...
//Paramètre d'entrée  : Aucun
//valeur retournée : Aucune 
function commanderColorB() {

    for (let i = 0; i < tabCurseurBleu.length; i++) {

        commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(tabCurseurBleu[i])] = 0;
        if (tabCurseurBleu[i] === bleu) {
            commandControlerDalle.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(bleu)] = parseInt(document.getElementById(bleu).value);
            console.log("Bleu  ::" + parseInt(document.getElementById(bleu).value));
        }
...
function sendCommandControlerDalle() {
    commandControlerDalle.command = "C_CommanderEspaceCouleur";
    ...
    if (canSend === true) {
        ...
        }, 20); //timer pour empecher le spam de trame sur le serveur
    }
...
function changeCaisson() {
    setTimeout(() => {
        ...
        commandControlerDalle.command = "C_DemanderCaissonEspaceCouleur";
...
function ConnexionAuServeurWebsocket() {
    ...
    ws.onmessage = function (evt) {
        ...
        if (messageFromServer.command === "C_reply_DemanderCaissonEspaceCouleur" || messageFromServer.command === "C_replyCommandAjustCursorEspaceColor") {
            if (messageFromServer.numeroCaisson === commandControlerDalle.numeroCaisson) {
                console.log(messageFromServer)
                chargerLoader();

                //Verifie si la dernière Couleur de curseur recut par le serveur est la même Couleur sur la quelle l'utilisateur interagit, 
                //Si c'est le cas, nous attribuons la valeur que le serveur a aux curseur de la page.
                //Pour recuperer la bonne valeur nous utilisons tabOrderColorDMX.indexOf(rouge) afin de récupéré le bon élement du tableau grâce a l'id.
               
                console.log('actu');
                if (commandControlerDalle.rouge === rouge) {
                    document.getElementById(rouge).value = messageFromServer.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(rouge)];
                    document.getElementById("valeurCurseur1").innerHTML = document.getElementsByTagName("input")[1].value + "%";
                }

                if (commandControlerDalle.vert === vert) {
                    document.getElementById(vert).value = messageFromServer.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(vert)];
                    document.getElementById("valeurCurseur2").innerHTML = document.getElementsByTagName("input")[2].value + "%";
                }

                if (commandControlerDalle.bleu === bleu) {
                    document.getElementById(bleu).value = messageFromServer.dataEspaceCouleurCIE[tabOrderColorDMX.indexOf(bleu)];
                    document.getElementById("valeurCurseur3").innerHTML = document.getElementsByTagName("input")[3].value + "%";

                }

                commandControlerDalle.dataEspaceCouleurCIE = messageFromServer.dataEspaceCouleurCIE;
                CalculMélangeRGB();
            }
        }
    }
